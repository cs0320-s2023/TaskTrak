  ...
  return (
    <Paper className="week-view">
      <Scheduler data={props.calendarItems} height={690}>
        <ViewState currentDate={props.currentDate} onCurrentDateChange={currentDateChange} />
        ...
        <AppointmentForm
          basicLayoutComponent={({ onFieldChange, appointmentData, ...bl_props }) => (
            <AppointmentForm.BasicLayout {...bl_props} appointmentData={appointmentData} onFieldChange={onFieldChange}>
              <AppointmentForm.Select
                {...props}
                value={appointmentData.prioritySelect}
                onValueChange={(nextValue: string | number) => {onFieldChange({ prioritySelect: nextValue });
            }}
